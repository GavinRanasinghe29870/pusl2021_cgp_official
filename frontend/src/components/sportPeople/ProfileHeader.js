            ...
            {editing ? (
              ...
              <>
                <h2 className="text-xl md:text-2xl font-semibold text-black flex items-center gap-2">
                  ...
                  {isOwner && <FiEdit className="text-gray-600 cursor-pointer" onClick={() => setEditing(true)} />}
                </h2>
                <p className="text-gray-600 flex items-center gap-1 mt-1">
                  <FaMapMarkerAlt className="text-blue-600" />
                  <span className="font-medium">{editableInfo.city || 'Unknown'}</span>
                  {isOwner && <FiEdit className="ml-1 text-gray-600 cursor-pointer" onClick={() => setEditing(true)} />}
                </p>
                <p className="text-gray-700 mt-1">
                  {editableInfo.sports || 'No sports added'}
                  {isOwner && <FiEdit className="ml-2 text-gray-600 cursor-pointer inline" onClick={() => setEditing(true)} />}
                </p>
              </>
            )}
          </div>
